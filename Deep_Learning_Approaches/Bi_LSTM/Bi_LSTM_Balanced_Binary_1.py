...
from Bi_LSTM.Bi_LSTM_Create_Model import create_model_Bi_LSTM,callback,plot_loss_accu

vocabulary_size=embedding_matrix.shape[0]

model_name = 'Models/Bi_LSTM/Model_Bi_LSTM_balanced_1_class_2.h5'
cb=callback(model_name=model_name)

model=create_model_Bi_LSTM(vocabulary_size,embedding_size,embedding_matrix,num_class=1)
history=model.fit(X_train,y_train,batch_size=512,epochs=10,validation_split=0.2,shuffle=True,callbacks=cb)

print("Saving Model...")
model_name = 'Models/Bi_LSTM/Balanced/Model_Bi_LSTM_balanced_1_class_2_sigmoid_binary_crossentropy.h5'#######################
model.save(model_name)

plot_loss_accu(history,lossLoc='Graph/Bi_LSTM/Balanced/Train_Val_Loss_balanced_1_class_2_sigmoid_binary_crossentropy',
               accLoc='Graph/Bi_LSTM/Balanced/Train_Val_acc_balanced_1_class_2_sigmoid_binary_crossentropy')


'''model_1=create_model_Bi_LSTM(vocabulary_size,embedding_size,embedding_matrix,num_class=1)
model_1.load_weights(model_name)
model_1.name = 'model_1' '''
model_1=load_model('Models/Bi_LSTM/Balanced/Model_Bi_LSTM_balanced_1_class_2_sigmoid_binary_crossentropy.h5')
...
import pickle
with open('Predictions/Bi_LSTM/Balanced/Bi_LSTM_balanced_1_class_2_sigmoid_binary_crossentropy.pickle','wb') as f:
    pickle.dump((y_test,pred,pred_binary),f)

pickle_load='Predictions/Bi_LSTM/Balanced/Bi_LSTM_balanced_1_class_2_sigmoid_binary_crossentropy.pickle'
